...
\begin{definition} A \textbf{gradual LNL-model} consists of
  ...
  \begin{enumerate}
  \item $F\star \lolli F\star$ is a retract of $F(\star \Rightarrow \star)$. That is, there exist morphisms $m_r : F(\star) \lolli F(\star) \mto F(\star \Rightarrow \star)$ and $m_l : F(\star \Rightarrow \star) \mto F(\star) \lolli F(star)$ with $m_r ; m_l = \id_{F(\star) \lolli F(\star)}$.
  \item $G(\star) \cong \star$
  \end{enumerate}

\end{definition}

The subscript on $\star$ shall be omitted below where the meaning is clear from the context.

We now show that $F$ preserves the retract properties of $\squash_S$ and $\split_S$ in $\cat{C}$ for $\star \Rightarrow \star$ and $\star \times \star$. That is, we shall show that $F(\star) \lolli F(\star)$ is a retract of $F\star$, as is $F\star \otimes F\star$.
\vspace{-15px}
